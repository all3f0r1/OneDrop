use milk_eval::{MilkEvaluator, eval_simple};
...
fn test_real_world_per_frame_equations() {
    ...
    assert!(wave_b > 0.0 && wave_b < 2.0, "wave_b out of expected range: {}", wave_b);
}
...
fn test_real_world_per_pixel_equations() {
    ...
    eval.eval_per_pixel(1.0, 1.0, 0.707, 0.785, &equations).unwrap();
    let zoom_edge = eval.context().get_var("zoom").unwrap();
    assert!(zoom_edge > 1.0, "Zoom should increase at edge: {}", zoom_edge);
}
...
fn test_complex_math_expressions() {
    ...
    let result = eval.eval("sin(time) * cos(time * 2) + sqrt(abs(time - 0.5))").unwrap();
...
fn test_conditional_like_behavior() {
    ...
    let result = eval.eval("min(1, max(0, (bass - 0.5) * 10)) * 0.5 + 0.5").unwrap();
    assert!(result > 0.5);
}
