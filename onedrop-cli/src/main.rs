...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Render { preset, frames, output, width, height } => {
...
fn cmd_info(preset_path: PathBuf) -> Result<()> {
    ...
    println!("\n--- Shaders ---");
    println!("Warp shader: {}", if preset.warp_shader.is_some() { "Yes" } else { "No" });
    println!("Composite shader: {}", if preset.comp_shader.is_some() { "Yes" } else { "No" });
...
fn cmd_list(directory: PathBuf) -> Result<()> {
    ...
    for (i, preset) in presets.iter().enumerate() {
        println!("  {}. {}", i + 1, preset.file_name().unwrap().to_string_lossy());
    }
