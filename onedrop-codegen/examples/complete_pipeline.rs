...
use onedrop_codegen::{ShaderGenerator, ShaderCompiler};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    preset.per_pixel_equations.push("x = x + 0.01*sin(time)".to_string());
    preset.per_pixel_equations.push("y = y + 0.01*cos(time)".to_string());
    println!("  ✓ Preset created with {} per-pixel equations\n", preset.per_pixel_equations.len());
    ...
    println!("    ... ({} more lines)", shader_source.lines().count() - 20);
    println!("---\n");
    ...
    println!("  Module entry points: {}", compiled.module.entry_points.len());
