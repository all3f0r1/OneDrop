...
impl SafePresetLoader {
    ...
    pub fn load_with_fallback<P: AsRef<Path>>(engine: &mut MilkEngine, path: P) -> Result<()> {
        ...
        match engine.load_preset(path_ref) {
            ...
            Err(e) => {
                log::error!("Failed to load preset {}: {}. Loading default preset.", 
                           path_ref.display(), e);
                ...
                match engine.load_default_preset() {
                    ...
                    Err(fallback_err) => {
                        ...
                        Err(EngineError::PresetLoadFailed(
                            format!("Both preset and fallback failed: {} / {}", e, fallback_err)
                        ))
                    }
                }
            }
    ...
    pub fn load_with_retry<P: AsRef<Path>>(
        ...
        for attempt in 0..=max_retries {
            match engine.load_preset(path_ref) {
                Ok(()) => {
                    if attempt > 0 {
                        log::info!("Successfully loaded preset {} after {} retries", 
                                  path_ref.display(), attempt);
                    }
                    return Ok(());
                }
                Err(e) => {
                    log::warn!("Attempt {}/{} failed to load preset {}: {}", 
                              attempt + 1, max_retries + 1, path_ref.display(), e);
                    last_error = Some(e);
        ...
        log::error!("All {} attempts failed for preset {}. Loading default preset.", 
                   max_retries + 1, path_ref.display());
    ...
    pub fn validate_preset<P: AsRef<Path>>(path: P) -> Result<()> {
        ...
        // Check file exists
        if !path_ref.exists() {
            return Err(EngineError::PresetLoadFailed(
    ...
    pub fn scan_directory<P: AsRef<Path>>(dir: P) -> Vec<std::path::PathBuf> {
        ...
        log::info!("Found {} valid presets in {}", valid_presets.len(), dir_ref.display());
        valid_presets
    }
}
