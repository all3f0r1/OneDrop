...
fn test_hardcut1_bass_detection() {
    ...
    assert_eq!(
        change,
        Some(PresetChange::Random),
        "Should trigger after min delay"
    );
}
...
fn test_hardcut2_treb_detection() {
    ...
    assert_eq!(
        change,
        Some(PresetChange::Random),
        "Should trigger after 0.5s delay"
    );
}
...
fn test_hardcut4_dual_threshold() {
    ...
    assert_eq!(
        change,
        Some(PresetChange::Random),
        "Should trigger immediately at treb > 8"
    );
    ...
    assert_eq!(
        change,
        Some(PresetChange::Random),
        "Should trigger again at treb > 8"
    );
}
...
fn test_hardcut6_special_preset() {
    ...
    assert_eq!(
        change,
        Some(PresetChange::Random),
        "Should load random preset at bass > 1.5"
    );
...
fn test_realistic_audio_scenario() {
    ...
    for (bass, mid, treb) in audio_sequence {
        if detector.should_change_preset(bass, mid, treb).is_some() {
            triggered_count += 1;
        }
    ...
    assert_eq!(
        triggered_count, 1,
        "Should trigger exactly once at the beat"
    );
}
