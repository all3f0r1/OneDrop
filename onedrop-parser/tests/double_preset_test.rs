use onedrop_parser::{parse_double_preset, BlendPattern};
...
fn test_parse_plasma_blend() {
    ...
    assert!(result.is_ok(), "Failed to parse plasma_blend.od2: {:?}", result.err());
    ...
    println!("   Preset A equations: {}", double.preset_a.per_frame_equations.len());
    println!("   Preset B equations: {}", double.preset_b.per_frame_equations.len());
}
