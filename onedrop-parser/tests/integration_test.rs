...
fn test_parse_single_preset() {
    let test_presets_dir = Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .unwrap()
        .join("test-presets");
...
fn test_parse_all_test_presets() {
        ...
        for entry in entries.flatten() {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("milk") {
                ...
                match fs::read_to_string(&path) {
                    Ok(content) => match parse_preset(&content) {
                        Ok(preset) => {
                            success_count += 1;
    ...
    assert!(
        success_count > 0,
        "Should parse at least one preset successfully"
    );
}
...
fn test_preset_structure_statistics() {
    ...
    if total > 0 {
        println!(
            "With per-frame equations: {} ({:.1}%)",
            with_per_frame,
            (with_per_frame as f64 / total as f64) * 100.0
        );
        println!(
            "With per-pixel equations: {} ({:.1}%)",
            with_per_pixel,
            (with_per_pixel as f64 / total as f64) * 100.0
        );
        println!(
            "With warp shader: {} ({:.1}%)",
            with_warp_shader,
            (with_warp_shader as f64 / total as f64) * 100.0
        );
        println!(
            "With comp shader: {} ({:.1}%)",
            with_comp_shader,
            (with_comp_shader as f64 / total as f64) * 100.0
        );
        println!(
            "With custom waves: {} ({:.1}%)",
            with_waves,
            (with_waves as f64 / total as f64) * 100.0
        );
        println!(
            "With custom shapes: {} ({:.1}%)",
            with_shapes,
            (with_shapes as f64 / total as f64) * 100.0
        );
    }
