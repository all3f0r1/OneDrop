...
fn test_preset(path: &Path, report: &mut CompatibilityReport) {
    ...
    let content = match fs::read_to_string(path) {
        Ok(c) => c,
        Err(e) => {
            report.parse_failures.push((filename, format!("Read error: {}", e)));
            return;
        }
    };
    ...
    let preset = match parse_preset(&content) {
        ...
        Err(e) => {
            report.parse_failures.push((filename, format!("Parse error: {:?}", e)));
            return;
        }
    };
    ...
    if eval_errors.is_empty() {
        ...
        report.eval_failures.push((filename, eval_errors.join("; ")));
    }
}
...
fn main() {
    ...
    println!("  Success: {} ({:.1}%)", report.parse_success, report.parse_success_rate());
    println!("  Failures: {}", report.parse_failures.len());
    println!();
    println!("Evaluation Results:");
    println!("  Success: {} ({:.1}% of parsed)", report.eval_success, report.eval_success_rate());
    println!("  Failures: {}", report.eval_failures.len());
    println!();
    println!("Overall Compatibility: {:.1}%", report.overall_success_rate());
    ...
    report_content.push_str(&format!("Parse success: {} ({:.1}%)\n", report.parse_success, report.parse_success_rate()));
    report_content.push_str(&format!("Eval success: {} ({:.1}%)\n", report.eval_success, report.eval_success_rate()));
    report_content.push_str(&format!("Overall: {:.1}%\n\n", report.overall_success_rate()));
